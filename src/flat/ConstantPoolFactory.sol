...
abstract contract PoolDeployer {
    ...
    function poolsCount(address token0, address token1)
        external
        view
...
interface IMasterDeployer {
    ...
    function deployPool(address factory, bytes calldata deployData)
        external
        returns (address);
}
...
library RebaseLibrary {
    ...
    function toBase(Rebase memory total, uint256 elastic)
        internal
        pure
    ...
    function toElastic(Rebase memory total, uint256 base)
        internal
        pure
...
contract ConstantProductPool is IPool, ERC20, ReentrancyGuard {
    ...
    function mint(bytes calldata data)
        public
        override
        nonReentrant
        returns (uint256 liquidity)
    {
        ...
        (uint256 fee0, uint256 fee1) = _nonOptimalMintFee(
            amount0,
            amount1,
            _reserve0,
            _reserve1
    ...
    {
        (address recipient, bool unwrapBento) = abi.decode(
            data,
            (address, bool)
        );
        (
        ...
        withdrawnAmounts[0] = TokenAmount({
            token: address(token0),
            amount: amount0
        });
        withdrawnAmounts[1] = TokenAmount({
    ...
    function burnSingle(bytes calldata data)
        ...
        returns (uint256 amountOut)
    {
        (address tokenOut, address recipient, bool unwrapBento) = abi.decode(
            data,
    ...
    function swap(bytes calldata data)
        ...
        returns (uint256 amountOut)
    {
        (address tokenIn, address recipient, bool unwrapBento) = abi.decode(
            data,
            (address, address, bool)
        );
        (
        ...
        returns (uint256 amountOut)
    {
        ...
        ) = abi.decode(data, (address, address, bool, uint256, bytes));
        (
            uint112 _reserve0,
            uint112 _reserve1,
            uint32 _blockTimestampLast
        ...
        unchecked {
            if (tokenIn == token0) {
                ...
                (uint256 balance0, uint256 balance1) = _balance();
                if (balance0 - _reserve0 < amountIn)
                ...
                ITridentCallee(msg.sender).tridentSwapCallback(context);
                (uint256 balance0, uint256 balance1) = _balance();
                if (balance1 - _reserve1 < amountIn)
                    revert InsufficientAmountIn();
                _update(
                    balance0,
                    balance1,
        ...
        if (_blockTimestampLast == 0) {
            ...
            if (
                ...
                unchecked {
                    ...
                    uint256 price0 = (uint256(_reserve1) << PRECISION) /
                        _reserve0;
                    price0CumulativeLast += price0 * timeElapsed;
                    uint256 price1 = (uint256(_reserve0) << PRECISION) /
                        _reserve1;
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1)
        internal
        returns (uint256 _totalSupply, uint256 computed)
    {
        ...
        if (_kLast != 0) {
            ...
            if (computed > _kLast) {
                ...
                uint256 numerator = _totalSupply *
                    (computed - _kLast) *
                    _barFee;
                uint256 denominator = (MAX_FEE - _barFee) *
                    computed +
                    _barFee *
                    _kLast;
    ...
    function _getAmountOut(
        ...
        amountOut =
            ...
            (reserveAmountIn * MAX_FEE + amountInWithFee);
    }
    ...
    function _getAmountIn(
        ...
        amountIn =
            (reserveAmountIn * amountOut * MAX_FEE) /
            ((reserveAmountOut - amountOut) * MAX_FEE_MINUS_SWAP_FEE) +
            1;
    }
    ...
    function getAmountIn(bytes calldata data)
        ...
        returns (uint256 finalAmountIn)
    {
        (address tokenOut, uint256 amountOut) = abi.decode(
            data,
            (address, uint256)
        );
        (uint112 _reserve0, uint112 _reserve1, ) = _getReserves();
...
contract ConstantProductPoolFactory is PoolDeployer {
    ...
    function deployPool(bytes memory _deployData)
        external
        returns (address pool)
    {
        ...
        pool = address(
            new ConstantProductPool{salt: salt}(
                _deployData,
